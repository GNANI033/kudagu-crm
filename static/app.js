...
function normalizeProductVariantInput(raw){
  const val=String(raw||'').trim();
  if(!val) return '';
  if(/^[0-9]*\.?[0-9]+$/.test(val)) return normalizeVariantValue(val,'g');
  const m=val.toLowerCase().match(/^([0-9]*\.?[0-9]+)\s*(kg|g|l|ml|pcs)$/);
  if(m) return normalizeVariantValue(m[1],m[2]);
  return normalizeVariantValue(val,'custom');
}
function buildExistingProductVariantRows(pid, sizes){
  return (Array.isArray(sizes)?sizes:[]).map((size,i)=>`<div class="product-variant-row">
    <input id="prod-var-${pid}-${i}" type="text" value="${esc(size)}" placeholder="e.g. 250g">
    <button class="del-btn" onclick="removeExistingProductVariantRow('${pid}',${i})">✕</button>
  </div>`).join('');
}
function getExistingProductVariantRows(pid){
  const rows=[]; let i=0;
  while(g(`prod-var-${pid}-${i}`)){
    const normalized=normalizeProductVariantInput(g(`prod-var-${pid}-${i}`)?.value||'');
    if(normalized) rows.push(normalized);
    i++;
  }
  const out=[]; const seen=new Set();
  rows.forEach((size)=>{ const key=size.toLowerCase(); if(seen.has(key)) return; seen.add(key); out.push(size); });
  return out;
}
function renderExistingProductVariantRows(pid, sizes){
  const wrap=g(`prod-var-rows-${pid}`); if(!wrap) return;
  wrap.innerHTML=buildExistingProductVariantRows(pid, sizes);
  refreshExistingProductVariantHint(pid);
}
function refreshExistingProductVariantHint(pid){
  const el=g(`prod-var-hint-${pid}`); if(!el) return;
  const sizes=getExistingProductVariantRows(pid);
  el.textContent=sizes.length?`${sizes.length} variant${sizes.length!==1?'s':''}: ${sizes.join(', ')}`:'Add at least one variant.';
  el.style.color=sizes.length?'var(--text-3)':'var(--amber)';
}
function addExistingProductVariantRow(pid){
  const input=g(`prod-var-new-${pid}`);
  const normalized=normalizeProductVariantInput(input?.value||'');
  if(!normalized){ toast('Enter a variant like 750g or 1kg','err'); return; }
  const rows=getExistingProductVariantRows(pid);
  if(rows.some((size)=>size.toLowerCase()===normalized.toLowerCase())){ toast('Variant already exists','err'); return; }
  rows.push(normalized);
  if(input) input.value='';
  renderExistingProductVariantRows(pid, rows);
}
function removeExistingProductVariantRow(pid, idx){
  const rows=getExistingProductVariantRows(pid);
  if(rows.length<=1){ toast('Keep at least one variant','err'); return; }
  renderExistingProductVariantRows(pid, rows.filter((_,i)=>i!==idx));
}
async function saveExistingProductVariants(pid){
  const prod=S.products.find(p=>p.id===pid); if(!prod) return;
  const sizes=getExistingProductVariantRows(pid);
  if(!sizes.length){ toast('Add at least one variant','err'); return; }
  try{
    const updated=await api.put(`/api/products/${pid}`,{sizes,pricing:prod.pricing||{}});
    const idx=S.products.findIndex(p=>p.id===pid); if(idx>=0) S.products[idx]=updated;
    syncPricingCalculatorState();
    toast('Product variants saved','ok');
    rSettings();
  }catch(e){ toast('Error: '+e.message,'err'); }
}
...
const PRICING_CALC_DEFAULT_INPUTS = {
  ...
  bulkPackagingPerKg:27, bulkShippingPerKg:65,
...
const PRICING_CALC_DEFAULT_VARIANTS = [
  {id:'base-250g',label:'250g',grams:250,discountPct:0,packagingCost:20,shippingCost:27.5,isBase:true},
  {id:'var-500g',label:'500g',grams:500,discountPct:0.085,packagingCost:40,shippingCost:55,isBase:false},
  {id:'var-1000g',label:'1000g',grams:1000,discountPct:0.085,packagingCost:80,shippingCost:110,isBase:false},
];
...
function pricingCalcDefaultIngredientSources(){ return {robusta:{inventoryProductId:'',inventoryProductName:''},arabica:{inventoryProductId:'',inventoryProductName:''},chicory:{inventoryProductId:'',inventoryProductName:''}}; }
function pricingCalcRoundUpToFive(value){
  const num=parseFloat(value||0)||0;
  if(num<=0) return 0;
  return Math.ceil(num/5)*5;
}
function pricingCalcDefaultProfile(name='Untitled Profile'){ return {id:0,name,inputs:{...PRICING_CALC_DEFAULT_INPUTS},ingredientSources:pricingCalcDefaultIngredientSources(),extraCosts:[],variants:PRICING_CALC_DEFAULT_VARIANTS.map(v=>({...v})),rows:pricingCalcProductRows(),roundToFive:false,nonBulkMarginMode:'markup',createdAt:0,updatedAt:0}; }
...
function pricingCalcMaterializeProfile(profile){
  ...
  const ingredientSources=base.ingredientSources&&typeof base.ingredientSources==='object'?base.ingredientSources:{};
  base.ingredientSources=pricingCalcDefaultIngredientSources();
  ['robusta','arabica','chicory'].forEach((key)=>{ const row=ingredientSources[key]&&typeof ingredientSources[key]==='object'?ingredientSources[key]:{}; base.ingredientSources[key]={inventoryProductId:String(row.inventoryProductId||''),inventoryProductName:String(row.inventoryProductName||'')}; });
  base.roundToFive=base.roundToFive===true;
  base.nonBulkMarginMode=base.nonBulkMarginMode==='effective'?'effective':'markup';
  ...
  const legacyPackagingPerKg=parseFloat(base.inputs?.nonBulkPackagingPerKg??80)||0;
  const legacyShippingPerKg=parseFloat(base.inputs?.nonBulkShippingPerKg??110)||0;
  base.variants=pricingCalcMarkSmallestVariantAsBase((Array.isArray(base.variants)&&base.variants.length?base.variants:PRICING_CALC_DEFAULT_VARIANTS).map((row,idx)=>{ const grams=parseFloat(row.grams||0)||0; const hasPackaging=Object.prototype.hasOwnProperty.call(row,'packagingCost'); const hasShipping=Object.prototype.hasOwnProperty.call(row,'shippingCost'); return {id:String(row.id||`variant-${idx+1}`),label:String(row.label||`Variant ${idx+1}`),grams,discountPct:parseFloat(row.discountPct||0)||0,packagingCost:hasPackaging?(parseFloat(row.packagingCost||0)||0):legacyPackagingPerKg*(grams/1000),shippingCost:hasShipping?(parseFloat(row.shippingCost||0)||0):legacyShippingPerKg*(grams/1000),isBase:!!row.isBase}; }));
  return base;
}
...
function pricingCalcComputed(profile){
  ...
  draft.rows.forEach((row)=>{ const raw=((row.robustaPct/100)*draft.inputs.robustaCostPerKg)+((row.arabicaPct/100)*draft.inputs.arabicaCostPerKg)+((row.chicoryPct/100)*draft.inputs.chicoryCostPerKg); const bulkCostPerKg=(raw+draft.inputs.processingCostPerKg+draft.inputs.bulkPackagingPerKg+draft.inputs.bulkShippingPerKg+bulkExtra); const bulk=draft.roundToFive?pricingCalcRoundUpToFive(bulkCostPerKg*(1+draft.inputs.bulkMarginPct)):bulkCostPerKg*(1+draft.inputs.bulkMarginPct); const sharedNonBulkPerGram=(raw+draft.inputs.processingCostPerKg+draft.inputs.baseTransportPerKg+nonBulkExtra)/1000; rows.push({rawCoffeeCostPerKg:raw,bulkCostPerKg,bulkMrpPerKg:bulk,bulkMarginRupees:bulk-bulkCostPerKg,bulkEffectiveMarginPct:bulk>0?((bulk-bulkCostPerKg)/bulk*100):0,variants:draft.variants.map((variant)=>{ const grams=parseFloat(variant.grams||0)||0; const discount=(parseFloat(variant.discountPct||0)||0); const cost=(grams*sharedNonBulkPerGram)+(parseFloat(variant.packagingCost||0)||0)+(parseFloat(variant.shippingCost||0)||0); let undiscountedMrp=draft.nonBulkMarginMode==='effective'?(cost/Math.max(0.000001,1-draft.inputs.nonBulkMarginPct)):(cost*(1+draft.inputs.nonBulkMarginPct)); if(draft.roundToFive) undiscountedMrp=pricingCalcRoundUpToFive(undiscountedMrp); const mrp=draft.roundToFive?pricingCalcRoundUpToFive(undiscountedMrp*(1-discount)):(undiscountedMrp*(1-discount)); const marginRupees=mrp-cost; return {id:variant.id,mrp,costPerPack:cost,marginRupees,effectiveMarginPct:mrp>0?(marginRupees/mrp*100):0}; })}); });
...
function pricingCalcValidation(profile){
  ...
  draft.variants.forEach((row)=>{ if((parseFloat(row.grams)||0)<=0) errors.push(`Variant ${row.label||row.id} must have grams greater than 0`); if((parseFloat(row.discountPct)||0)<0) errors.push(`Variant ${row.label||row.id} discount cannot be negative`); if((parseFloat(row.packagingCost)||0)<0) errors.push(`Variant ${row.label||row.id} packaging cannot be negative`); if((parseFloat(row.shippingCost)||0)<0) errors.push(`Variant ${row.label||row.id} shipping cannot be negative`); });
...
function pricingCalcInventorySelect(key,label){
  const source=PRICING_CALC_STATE.draft?.ingredientSources?.[key]||{};
  return `<div class="fg"><label>${label} Ingredient</label><select onchange="updatePricingCalcIngredientSource('${key}',this.value)">${inventoryProductOptions(source.inventoryProductId||'')}</select></div>`;
}
function rPricingCalculator(){
  ...
  const variantRows=draft.variants.map((v,idx)=>`<tr><td><input type="text" value="${esc(v.label)}" oninput="updatePricingCalcVariant(${idx},'label',this.value)"></td><td><input type="number" min="1" step="1" value="${v.grams}" oninput="updatePricingCalcVariant(${idx},'grams',this.value)"></td><td><div class="input-prefix"><span>₹</span><input type="number" min="0" step="0.01" value="${v.packagingCost}" oninput="updatePricingCalcVariant(${idx},'packagingCost',this.value)"></div></td><td><div class="input-prefix"><span>₹</span><input type="number" min="0" step="0.01" value="${v.shippingCost}" oninput="updatePricingCalcVariant(${idx},'shippingCost',this.value)"></div></td><td><div class="input-prefix"><span>%</span><input type="number" min="0" step="0.01" value="${pricingCalcPercentValue(v.discountPct)}" oninput="updatePricingCalcVariant(${idx},'discountPct',this.value)"></div></td><td><button type="button" class="btn btn-g btn-xs" data-pc-action="remove-variant" data-pc-index="${idx}">Remove</button></td></tr>`).join('');
...
function refreshPricingCalcComputed(){
  ...
  draft.rows.forEach((row,idx)=>{
    const total=(parseFloat(row.robustaPct)||0)+(parseFloat(row.arabicaPct)||0)+(parseFloat(row.chicoryPct)||0);
    const totalEl=g(`pc-total-${idx}`);
    if(totalEl){
      totalEl.textContent=`${total.toFixed(2)}%`;
      totalEl.style.color=(total>=99.99&&total<=100.01)?'var(--green)':'var(--red)';
    }
    const result=computed.rows[idx];
    if(!result) return;
    if(g(`pc-raw-${idx}`)) g(`pc-raw-${idx}`).textContent=pricingCalcFormatCurrency(result.rawCoffeeCostPerKg);
    if(g(`pc-bulk-${idx}`)) g(`pc-bulk-${idx}`).innerHTML=`<div>${pricingCalcFormatCurrency(result.bulkMrpPerKg)}</div><div style="font-size:11px"><span style="color:${result.bulkMarginRupees>=0?'var(--green)':'var(--red)'}">${pricingCalcFormatCurrency(result.bulkMarginRupees)}</span> <span style="color:var(--text-3)">|</span> <span style="color:var(--red)">${pricingCalcFormatCurrency(result.bulkCostPerKg)}</span> <span style="color:var(--text-3)">|</span> <span style="color:${result.bulkMarginRupees>=0?'var(--green)':'var(--red)'}">${result.bulkEffectiveMarginPct.toFixed(1)}%</span></div>`;
    result.variants.forEach((variant)=>{
      const el=g(`pc-var-${idx}-${variant.id}`);
      if(el) el.innerHTML=`<div>${pricingCalcFormatCurrency(variant.mrp)}</div><div style="font-size:11px"><span style="color:${variant.marginRupees>=0?'var(--green)':'var(--red)'}">${pricingCalcFormatCurrency(variant.marginRupees)}</span> <span style="color:var(--text-3)">|</span> <span style="color:var(--red)">${pricingCalcFormatCurrency(variant.costPerPack)}</span> <span style="color:var(--text-3)">|</span> <span style="color:${variant.marginRupees>=0?'var(--green)':'var(--red)'}">${variant.effectiveMarginPct.toFixed(1)}%</span></div>`;
    });
  });
...
function togglePricingCalcRoundToFive(checked){ PRICING_CALC_STATE.draft.roundToFive=!!checked; refreshPricingCalcComputed(); }
function updatePricingCalcNonBulkMarginMode(value){ PRICING_CALC_STATE.draft.nonBulkMarginMode=value==='effective'?'effective':'markup'; refreshPricingCalcComputed(); }
function updatePricingCalcIngredientSource(key, inventoryProductId){
  if(!PRICING_CALC_STATE.draft.ingredientSources) PRICING_CALC_STATE.draft.ingredientSources=pricingCalcDefaultIngredientSources();
  const item=(inventorySnapshot||[]).find((row)=>row.id===inventoryProductId);
  PRICING_CALC_STATE.draft.ingredientSources[key]={inventoryProductId:String(inventoryProductId||''),inventoryProductName:String(item?.name||inventoryProductId||'')};
}
...
async function savePricingCalcProfile(){ if(!canManagePricingCalculator()){ toast('Settings access is restricted','err'); return; } const draft=pricingCalcMaterializeProfile(PRICING_CALC_STATE.draft); const errors=pricingCalcValidation(draft); if(errors.length){ toast(errors[0],'err'); return; } try{ const payload={name:draft.name,inputs:draft.inputs,ingredientSources:draft.ingredientSources,extraCosts:(draft.extraCosts||[]).filter((row)=>String(row.label||'').trim() || (parseFloat(row.amount||0)||0)>0),variants:draft.variants,rows:draft.rows.filter((row)=>!row.missingProduct).map((row)=>({crmProductId:row.crmProductId,productNameSnapshot:row.productNameSnapshot,robustaPct:row.robustaPct,arabicaPct:row.arabicaPct,chicoryPct:row.chicoryPct,enabled:row.enabled!==false})),roundToFive:draft.roundToFive===true,nonBulkMarginMode:draft.nonBulkMarginMode}; const saved=draft.id?await api.put(`/api/pricing-calculator/profiles/${draft.id}`,payload):await api.post('/api/pricing-calculator/profiles',payload); S.pricingCalculatorProfiles=[saved,...(S.pricingCalculatorProfiles||[]).filter((row)=>row.id!==saved.id)].sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0)); PRICING_CALC_STATE.activeId=saved.id; PRICING_CALC_STATE.draft=pricingCalcMaterializeProfile(saved); toast('Pricing profile saved','ok'); rPricingCalculator(); }catch(e){ toast('Error: '+e.message,'err'); } }
...
async function publishPricingCalcToProducts(){
  ...
  try{
    const payload={
      name:draft.name,
      inputs:draft.inputs,
      ingredientSources:draft.ingredientSources,
      extraCosts:(draft.extraCosts||[]).filter((row)=>String(row.label||'').trim() || (parseFloat(row.amount||0)||0)>0),
      variants:draft.variants,
      roundToFive:draft.roundToFive===true,
      nonBulkMarginMode:draft.nonBulkMarginMode,
...
Object.assign(window,{addPricingCalcExtraCost,updatePricingCalcExtraCost,removePricingCalcExtraCost,addPricingCalcVariant,removePricingCalcVariant,updatePricingCalcVariant,newPricingCalcProfile,duplicatePricingCalcProfile,deletePricingCalcProfile,savePricingCalcProfile,publishPricingCalcToProducts,updatePricingCalcInput,updatePricingCalcName,togglePricingCalcRoundToFive,updatePricingCalcNonBulkMarginMode,updatePricingCalcIngredientSource,updatePricingCalcRow,togglePricingCalcRow,loadPricingCalcProfile,saveProductCategory});

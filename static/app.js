...
function pricingCalcComputed(profile){
  ...
  draft.rows.forEach((row)=>{ const raw=((row.robustaPct/100)*draft.inputs.robustaCostPerKg)+((row.arabicaPct/100)*draft.inputs.arabicaCostPerKg)+((row.chicoryPct/100)*draft.inputs.chicoryCostPerKg); const bulkCostPerKg=(raw+draft.inputs.processingCostPerKg+draft.inputs.bulkPackagingPerKg+draft.inputs.bulkShippingPerKg+bulkExtra); const bulk=draft.roundToFive?pricingCalcRoundUpToFive(bulkCostPerKg*(1+draft.inputs.bulkMarginPct)):bulkCostPerKg*(1+draft.inputs.bulkMarginPct); const nonBulkCostPerKg=(raw+draft.inputs.processingCostPerKg+draft.inputs.nonBulkPackagingPerKg+draft.inputs.nonBulkShippingPerKg+draft.inputs.baseTransportPerKg+nonBulkExtra); const nonBulkCostPerGramBeforeMargin=nonBulkCostPerKg/1000; const base=(draft.nonBulkMarginMode==='effective'?(nonBulkCostPerKg/Math.max(0.000001,1-draft.inputs.nonBulkMarginPct)):nonBulkCostPerKg*(1+draft.inputs.nonBulkMarginPct))/1000; rows.push({rawCoffeeCostPerKg:raw,bulkCostPerKg,bulkMrpPerKg:bulk,bulkMarginRupees:bulk-bulkCostPerKg,bulkEffectiveMarginPct:bulk>0?((bulk-bulkCostPerKg)/bulk*100):0,variants:draft.variants.map((variant)=>{ const grams=parseFloat(variant.grams||0)||0; const discount=(parseFloat(variant.discountPct||0)||0); const baseMrp=grams*base*(1-discount); const mrp=draft.roundToFive?pricingCalcRoundUpToFive(baseMrp):baseMrp; const cost=grams*nonBulkCostPerGramBeforeMargin; const marginRupees=mrp-cost; return {id:variant.id,mrp,costPerPack:cost,marginRupees,effectiveMarginPct:mrp>0?(marginRupees/mrp*100):0}; })}); });
